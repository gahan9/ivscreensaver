...
def custom_log(txt, loglevel=xbmc.LOGDEBUG):
...
class Settings():
    ...
    def getScreensaverFolder():
        ...
        if screenFolder in [None, ""]:
            ...
            # Make sure the screensaver folder exists, if not, create it
    ...
    def get_current_week_even_odd():
            ...
            if ADDON.getSetting('odd_even_is_enabled') == 'true':
                ...
                if (int(check_week) % 2) == 0:
                    ...
                    # xbmc.log("ODD", 2)
                    return ADDON.getSetting('select_media_odd')
            else:
                both_list = [ADDON.getSetting('select_media_even'), ADDON.getSetting('select_media_odd')]
                xbmc.log("Feature is disable >>>>>>>@ :" + str(both_list), 2)
